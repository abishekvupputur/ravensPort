...
- **A bridge can carry its own custom headers**, sent on every call it makes — set them on the
  selected bridge's panel on the API to MCP tab. For a header the whole upstream API demands
  unconditionally rather than per operation (GitHub's REST API refuses every request with no
  `User-Agent`, for instance) — set it once here instead of repeating it in every tool. A tool's own
  manifest header for the same name still wins if it sets one; the same reserved names a manifest
  cannot write, this cannot either.
