...
| Asset | For |
|---|---|
| `RavensPort-Setup-4.4.1.exe` | installing outside the Store |
| `RavensPort-4.4.1.msix` | uploading to Partner Center |

Download the `.msix` from the release and upload it in Partner Center.

The version comes off the tag, so `v4.4.1` produces a `4.4.1.0` package. MSIX wants four parts and
...
```bash
gh attestation verify RavensPort-4.4.1.msix --repo abishekvupputur/ravensPort
```
...
```powershell
Import-Certificate -FilePath packaging/obj/RavensPort-4.4.1-signed.cer `
  -CertStoreLocation Cert:\LocalMachine\TrustedPeople
Add-AppxPackage -Path packaging/obj/RavensPort-4.4.1-signed.msix
```
