...
public static partial class VaultProbe
{
    ...
    /// searches read a per-user install out of. Windows carries the extension; nothing else does.
    ///
    ...
    private static string OnePasswordExeName => OperatingSystem.IsWindows() ? OpExe : "op";

    private static string ProtonPassExeName => OperatingSystem.IsWindows() ? ProtonPassExe : "pass-cli";
